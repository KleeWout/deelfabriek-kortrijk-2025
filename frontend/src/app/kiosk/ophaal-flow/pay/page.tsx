...
export default function PayconiqPayPage() {
  ...
  useEffect(() => {
    const initializePayment = async () => {
      try {
        const payment = await createPayconiqPayment(amount, "Deelfabriek Kortrijk - Item ophalen");
        ...
        <div className="flex items-center gap-10 ml-auto">
          ...
          <div className="flex items-center gap-3 opacity-60">
            <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center">
              <MagnifyingGlass size={28} weight="regular" className="text-[var(--color-primarygreen-1)]" />
            </div>
            <div>
              <div className="font-bold text-[var(--color-primarygreen-1)] text-lg">Stap 1</div>
              <div className="text-xs text-gray-700 font-semibold">Zoek en selecteer</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 opacity-60">
            <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center">
              <User size={28} weight="regular" className="text-[var(--color-primarygreen-1)]" />
            </div>
            <div>
              <div className="font-bold text-[var(--color-primarygreen-1)] text-lg">Stap 2</div>
              <div className="text-xs text-gray-700 font-semibold">Contactgegevens</div>
            </div>
          </div>
          <div className="w-8 h-0.5 bg-gray-300 mx-2" />
          {/* Step 3 - Active */}
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-[var(--color-primarygreen-1)] text-lg">Stap 3</div>
              <div className="text-xs text-gray-700 font-semibold">Betaling</div>
            </div>
          </div>
        </div>
      </div>
          ...
          {!paid ? (
            <div className="flex flex-col items-center justify-center px-8 py-8 gap-6">
              <div className="flex items-center gap-3 w-full mb-2">
                <Image src="/payconiq.png" alt="Payconiq" width={40} height={18} className="mb-2" />
              ...
              <div className="flex justify-center items-center w-full mt-2 mb-4">
                {qrCode ? (
                  <div className="bg-white border-2 border-gray-400 w-44 h-44 rounded-lg flex items-center justify-center overflow-hidden">
                    <Image src={qrCode} alt="Payconiq QR Code" width={176} height={176} className="object-contain" />
                  </div>
                ) : (
              ...
              <button className="w-full max-w-[220px] py-3 rounded-lg border-2 border-[var(--color-primarygreen-1)] text-[var(--color-primarygreen-1)] text-lg font-bold bg-white hover:bg-[var(--color-primarygreen-1)] hover:text-white transition" onClick={() => router.back()}>
                Annuleren
              </button>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center px-8 py-10 gap-8 flex-1">
              <LottiePlayer loop={false} play animationData={animationData} style={{ width: 180, height: 180 }} />
              <div className="text-2xl font-bold text-[var(--color-primarygreen-1)] text-center">Betaling succesvol</div>
