...
export default function LockersPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const availableItems = itemsData.filter(
          (item) => item.lockerId === null || item.lockerId === undefined
        );

        // If we're currently editing a locker, make sure its item is included in the list
        if (editingLocker && editingLocker.itemId) {
          const currentItemExists = availableItems.some(
            (item) => item.id === editingLocker.itemId
          );
  ...
  const handleSave = async () => {
    try {
      ...
      const isRemovingItem =
        editingLocker && editingLocker.itemId && !formData.itemId;
      ...
      const lockerData = {
        ...
        status: status,
        itemTitle: formData.itemId
          ? items.find((i) => i.id === formData.itemId)?.title
          : undefined,
      };
      ...
      if (editingLocker) {
        ...
        if (isRemovingItem) {
          console.log(
            `Item ${editingLocker.itemTitle} removed from locker ${editingLocker.lockerNumber}`
          );
        }
      } else {
        // Add new locker
  ...
  const handleInputChange = (field: keyof typeof formData, value: any) => {
    setFormData((prev) => {
      const updatedData = {
        ...prev,
        [field]:
          field === "itemId" ? (value === "" ? null : parseInt(value)) : value,
  ...
  const validateForm = () => {
    ...
    const existingLocker = lockers.find(
      (l) =>
        l.lockerNumber === formData.lockerNumber &&
        (!editingLocker || l.id !== editingLocker.id)
    );
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">
                {openingSuccess === null ? (
                  ...
                  <>Lockers succesvol geopend</>
                ) : (
                  <>Fout bij openen van lockers</>
                )}
              </h3>
              ...
              {openingSuccess === null && !loading && (
                <>
                  <p className="mb-6 text-gray-600">
                    Weet u zeker dat u {selectedLockers.length} locker
                    {selectedLockers.length > 1 ? "s" : ""} wilt openen?
                  </p>
                  <div className="flex justify-end space-x-3">
                    <button
                      className="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors"
                      onClick={() => setShowOpenConfirmation(false)}
                    >
                      Annuleren
                    </button>
                    <button
                      className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                      onClick={confirmOpenLockers}
                    >
                      Bevestigen
                    </button>
                  </div>
              ...
              {openingSuccess === true && (
                <div className="text-center py-4">
                  ...
                  <p className="text-green-600">
                    De geselecteerde lockers zijn succesvol geopend.
                  </p>
                </div>
              )}
              ...
              {openingSuccess === false && (
                <div className="text-center py-4">
                  ...
                  <p className="text-red-600 mb-4">
                    Er is een fout opgetreden bij het openen van de lockers.
                  </p>
          ...
          <div className="flex space-x-3">
            {selectedLockers.length > 0 && (
              <button
                ...
                onClick={handleOpenSelectedLockers}
                disabled={loading}
              >
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {lockers.map((locker) => (
                    <tr
                      key={locker.id}
                      className={
                        selectedLockers.includes(locker.id) ? "bg-blue-50" : ""
                      }
                    >
                      <td className="px-6 py-4 whitespace-nowrap w-8">
                        <input
                          ...
                          onChange={() => handleToggleSelectLocker(locker.id)}
                        />
                      </td>
                          ...
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            {
                              ...
                              locker.status ||
                                (locker.itemId ? "Bezet" : "Beschikbaar")
                            ]
                          }`}
                        >
                          {locker.status ||
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          className="text-[#004431] hover:text-[#003422] mr-2"
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-gray-700">
              {editingLocker ? "Locker bewerken" : "Locker aanmaken"}
            </h2>
          </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("lockerNumber", parseInt(e.target.value))
                }
            ...
            <div>
                  ...
                  <>
                    ...
                    {items.map((item) => (
                      <option
                        ...
                        value={item.id}
                        className={
                          editingLocker && editingLocker.itemId === item.id
                            ? "font-medium text-green-700 bg-green-100"
                            : ""
                        }
                      >
                        {item.title}
              ...
              {items.length === 0 && !loading && (
                <p className="mt-1 text-sm text-amber-600">
                  ...
                  <span className="font-light italic ml-1">
                    (Alleen items zonder toegewezen locker worden getoond)
                  </span>
                </p>
              )}
            </div>{" "}
