...
export default function OphaalFlowPage() {
  ...
  const accesories = Array.isArray(reservationData?.item?.accesories)
    ? reservationData.item.accesories.join(", ")
    : reservationData?.item?.accesories || "Geen accessoires";
  ...
  return (
    <div className="min-h-screen bg-[var(--color-primarybackground)] flex flex-col px-8 py-6">
      ...
      {/* Main content */}
      <div
        className="flex flex-row justify-center gap-10 w-full items-start"
      ...
      >
        ...
        <div className="relative flex-1 bg-white/90 border-2 border-[var(--color-secondarygreen-1)] rounded-2xl shadow-lg flex min-h-[280px] max-h-[440px] max-w-[1000px] p-0 overflow-hidden items-center">
          {/* Foto links */}{" "}
          <div
            className={`flex items-center justify-center bg-white h-full p-8 m-8 min-w-[320px] max-h-[240px] ${gradientClass} rounded-lg`}
          >
            <Image
              src={imageSrc}
              alt={reservationData?.item?.title || "Item"}
              width={260}
              height={180}
          ...
          <div className="flex flex-col flex-1 gap-2 min-w-0 p-8 pr-12 justify-center h-full">
            ...
            <div className="flex items-center gap-2 text-base text-[var(--color-primarytext-1)] mb-1">
              ...
              <span className="leading-normal pt-[1px]">
                ...
                <span className="font-bold">
                  {new Date(
                    Date.now() +
                      reservationData?.weeks * 7 * 24 * 60 * 60 * 1000
            ...
            <div className="flex items-center gap-2 text-base text-[var(--color-primarytext-1)] mb-1">
              ...
              <span>
                Pickup Code:{" "}
                <strong>{reservationData?.pickupCode || "------"}</strong>
              </span>
            </div>
            ...
            {reservationData?.item?.weight !== null &&
              reservationData?.item?.weight !== undefined && (
                <div className="text-base text-[var(--color-primarytext-1)] mb-1 flex gap-2">
                  <Scales
                    size={20}
                    className="text-[var(--color-primarygreen-1)]"
                  ...
                  <span>
                    ...
                    {reservationData.item.weight} kg
                  </span>
                </div>
              )}
        ...
        <div className="w-[340px] bg-white border-2 border-[var(--color-secondarygreen-1)] rounded-2xl p-7 flex flex-col gap-5 shadow-xl min-h-[320px] justify-start">
          ...
          <div className="flex flex-col gap-1 text-base text-[var(--color-primarytext-1)]">
            <div className="flex justify-between">
              <span>Locker</span>
              <span className="font-bold">
                {reservationData?.lockerNumber || "--"}
              </span>
            </div>{" "}
            <div className="flex justify-between">
              <span>Reserveringsduur</span>
              <span className="font-bold">
                {reservationData?.weeks || 0}{" "}
                {(reservationData?.weeks || 0) > 1 ? "weken" : "week"}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Code</span>
              <span className="font-bold">
                {reservationData?.pickupCode || "------"}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Item</span>
