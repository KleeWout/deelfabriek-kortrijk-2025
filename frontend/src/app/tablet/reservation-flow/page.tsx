  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f3f6f8]">
      ...
      <div className="flex items-center justify-between w-full max-w-7xl mx-auto mt-10 mb-8 px-8">
        <button
          className="rounded-lg p-3 bg-white shadow hover:bg-gray-100 transition border border-gray-200"
          onClick={() => router.back()}
          aria-label="Terug"
        >
          <ArrowLeft
        ...
        <h1
          className="text-4xl font-extrabold tracking-wide text-gray-700 mx-auto"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-10">
          {staticItems.map((item) => (
            <ItemCard
              ...
              status={item.status}
              index={item.id}
              onClick={() => router.push(`/tablet/reservation-flow/${item.id}`)}
            />
          ))}
        </div>
      </div>
