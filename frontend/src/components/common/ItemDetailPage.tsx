...
export default function ItemDetailPage() {
  ...
  useEffect(() => {
    ...
    if (item && item.imageSrc) {
      const url = `http://172.30.82.148:3001/photo?src=${encodeURIComponent(item.imageSrc)}`;
      fetch(url)
        .then((res) => res.blob())
        .then((blob) => {
  ...
  // Use fetchedImg if available, otherwise fallback
  const imageSrc = fetchedImg || "/placeholder-image.jpg";
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex flex-col lg:flex-row lg:gap-8">
            <div className="w-full lg:w-1/2 mb-4">
              <div
                className={`${gradientClass} p-4 rounded-lg flex items-center justify-center h-[350px]`}
              >
                <Image
                  src={imageSrc}
                  width={350}
